...
pub fn get_recent_activity_response_decoder() -> decode.Decoder(
  GetRecentActivityResponse,
) {
  use recent_activity <- decode.field(
    "recentActivity",
    decode.list(activity_entry_decoder()),
  )
  decode.success(GetRecentActivityResponse(recent_activity: recent_activity))
}

pub fn get_recent_activity_response_to_json(
  input: GetRecentActivityResponse,
) -> json.Json {
  json.object([
    #(
      "recentActivity",
      json.array(from: input.recent_activity, of: activity_entry_to_json),
    ),
  ])
}
...
pub fn parse_get_recent_activity_response(
  body: String,
) -> Result(GetRecentActivityResponse, String) {
