...
// Use AggregateResult from lexicon_graphql/aggregate_types
  ...
  let select_parts =
    ...
    |> list.index_map(fn(field, index) {
      ...
      case field {
        ...
        TruncatedField(f, interval) -> build_date_truncate_select(f, interval, field_name)
      }
    })
...
fn build_date_truncate_select(
  ...
  case interval {
    ...
    Week ->
      "strftime('%Y-W%W', " <> field_ref <> ") as " <> alias
